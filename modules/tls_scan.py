...
class TLSScanner(BaseModule):
    ...
    def _check_certificate(self, host, port, timeout) -> list:
        ...
        # Hostname + chain validation already happened above: ssl.create_default_context()
        # defaults to check_hostname=True and verify_mode=CERT_REQUIRED, so wrap_socket()
        # would have already raised SSLCertVerificationError on a mismatch. Reaching this
        # point means both checks passed.
